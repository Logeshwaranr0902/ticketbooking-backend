...
public class TheaterMapper {
    ...
    private Screen toScreenEntity(ScreenRequest screenReq, Theater theater) {
        ...
        screen.setSeats(generateSeats(screen, screenReq.getRows(), screenReq.getSeatsPerRow()));
        return screen;
    }

    private List<Seat> generateSeats(Screen screen, int rows, int seatsPerRow) {
        List<Seat> seats = new ArrayList<>();
        for (int r = 1; r <= rows; r++) {
            String rowLabel = String.valueOf((char) ('A' + r - 1));
            for (int s = 1; s <= seatsPerRow; s++) {
                Seat seat = new Seat();
                seat.setSeatNumber(rowLabel + s); // e.g., A1, A2...
                seat.setRowNumber(r);
